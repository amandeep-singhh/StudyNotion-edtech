        ...
        <>
            <div className='w-[100%] mx-auto  flex flex-col gap-16 min-h-[30rem] bg-richblack-900'>
                ...
                <div className='flex flex-col items-center gap-2'>
                    ...
                    <p className='font-medium text-richblack-300 text-center '>Learn to Build Anything You Can Imagine</p>
                </div>
                {/* tab names */}
                <div
                    ref={tabsContainer}
                    className='flex md:gap-2v gap-1 text-richblack-100 bg-richblack-800 max-w-[95%] md:w-fit md:px-3 md:py-2 p-1 rounded-full mx-auto overflow-x-scroll no-scrollbar relative'>
                    <div
                        ref={divRef}
                        className='absolute customGradient-Bg rounded-full  inset-[8px]  selectedTab ' ></div>
                    {tabsName.map((tab, index) => (
                        <div key={index} className={`z-20 text-sm md:text-base whitespace-nowrap  py-1 px-2 md:px-4 md:py-2 ${tab == currentTab ? 'text-richblack-900 font-semiboldn active' : 'hover:text-richblack-400'} transition-all duration-200 cursor-pointer rounded-full `}
                            onClick={(e) => {
            ...
            <div className='flex items-center justify-center w-full h-80 bg-[url("./assets/Images/bghome.svg")] relative' >
                ...
                <div className='flex md:gap-3 gap-1 mt-20 flex-col md:flex-row items-center'>
